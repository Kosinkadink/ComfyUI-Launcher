...
async function scanCustomNodes(installPath) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      dirEntries.push(nodePath);
    }
    const identified = await Promise.all(dirEntries.map((p) => identifyNode(p)));
    for (const node of identified) {
      nodes.push({ ...node, enabled: true });
    }
  } catch {}
  ...
  try {
    if (await exists(disabledDir)) {
      ...
      const disabledPaths = entries
        .filter((e) => e.isDirectory())
        .map((e) => path.join(disabledDir, e.name));
      const disabledNodes = await Promise.all(disabledPaths.map((p) => identifyNode(p)));
      for (const node of disabledNodes) {
        nodes.push({ ...node, enabled: false });
      }
    }
  } catch {}
