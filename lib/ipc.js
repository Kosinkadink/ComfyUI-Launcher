...
function register({ onLaunch, onStop, onComfyExited, onComfyRestarted, onLocaleChanged } = {}) {
  ...
  ipcMain.handle("run-action", async (_event, installationId, actionId, actionData) => {
    ...
    const update = (data) => installations.update(installationId, data);
    const actionCache = createCache(settings.get("cacheDir"), settings.get("maxCachedFiles"));
    try {
      return await resolveSource(inst.sourceId).handleAction(actionId, inst, actionData, { update, sendProgress, sendOutput, download, cache: actionCache, extract });
    } catch (err) {
      return { ok: false, message: err.message };
    }
  });
