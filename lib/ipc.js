...
const os = require("os");
const { spawnProcess, waitForPort, waitForUrl, killProcessTree, killByPort } = require("./process");
...
function register({ onLaunch, onStop, onComfyExited, onComfyRestarted } = {}) {
  ...
  ipcMain.handle("restart-core", () => {
    // ComfyUI-Manager calls electronAPI.restartApp() — flag restart then kill to trigger respawn
    if (_runningProc && !_runningProc.killed) {
      _restartRequested = true;
      killProcessTree(_runningProc);
    }
  });
  ...
  ipcMain.handle("run-action", async (_event, installationId, actionId, actionData) => {
    ...
    if (actionId === "launch") {
      ...
      function attachExitHandler(p) {
        p.on("exit", () => {
          const shouldRestart = _restartRequested || checkRebootMarker(sessionPath);
